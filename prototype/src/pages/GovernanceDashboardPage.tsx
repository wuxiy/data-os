...
import { governanceMetrics } from '../data/mock'
...
export function GovernanceDashboardPage({ onOpenChain, onNavigate, onUnavailable }: Props) {
  return (
    <div className={styles.page}>
      <PageHeader title="治理驾驶舱" />
        ...
        <div className={styles.twoColumns}>
          ...
          <section className={styles.panel}>
            ...
            <ol className={styles.ranking}>
              ...
              ].map(([system, owner, value], index) => <li key={system}><span className={styles.rank}>{index + 1}</span><div className={styles.rankBody}><strong>{system}</strong><span>{owner}</span></div><span className={styles.rankValue}>{value}</span></li>)}
            </ol>
          </section>
        </div>
        <section className={styles.tablePanel}>
          ...
          <div className={styles.tableScroll}>
            <table className={styles.table}>
              ...
              <tbody>
                <tr onClick={onOpenChain}><td>LIS 检验结果及时率下降</td><td>检验主题 / 38 张表</td><td>检验科</td><td>08-02 18:00</td><td><StatusTag tone="danger">逾期</StatusTag></td></tr>
