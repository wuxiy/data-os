...
export function ProductScopeNotice() {
  return (
    <aside className={styles.notice} role="note" aria-label="首期产品范围">
      ...
      <p>
        数据接入、采集运行、治理问题、质量复检和通知已接入控制面；患者主索引（MPI）、资产目录、分析看板、AI Ready 评估、对外数据服务、数据标准中心与管理驾驶舱/运营中心已接入控制面；标准映射、交付中心与受控智能问数（Beta，仅已验证问题，不做任意自然语言转 SQL）已接入控制面，生产模式不会把保存、确认或生成提示当作真实业务副作用。
      </p>
    </aside>
  )
}
