...
治理驾驶舱已通过 `src/data/controlPlane.ts` 接入控制面；数据接入页同样读取数据源与采集任务 API；质量闭环页读取治理问题列表、责任链详情、处理说明和复检事件 API。数据接入和质量闭环在控制面不可用时明确展示不可用空态，不用演示状态冒充真实数据。其余页面当前数据位于 `src/data/mock.ts`。接入后端时，建议保持页面组件的展示模型不变，将 mock 数据替换为按页面聚合的 BFF/API 响应，避免前端直接拼接五类治理来源。
