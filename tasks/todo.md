...
# 2026-09-20 下一阶段功能补齐规划（G21–G26）

## 目标

在不启动 H4/H5、不新增微服务的前提下，把门户中仍属原型或无入口的数据标准、标准映射、运营中心、交付中心和智能问数补成真实、可审计、可独立验收的功能闭环。

## 规划任务

- [x] 回顾历史教训、领域词汇、G1–G20 验收事实和延期台账
- [x] 复核六个子工程、门户真实/演示边界、当前 CI 与开发环境阻断
- [x] 确定先执行 G21 功能可信基线，再补五个产品面的顺序
- [x] 固化 G22 标准中心的版本、评审、发布、影响和 FHIR 导出边界
- [x] 固化 G23 映射治理、聚合验证、治理导航和 AI Ready 覆盖率接缝
- [x] 固化 G24 管理驾驶舱/运营中心、G25 交付证据、G26 受控问数方案
- [x] 明确 API、迁移、权限、失败态、规模、回滚和全量验证要求
- [x] 产出正式计划 `docs/feature-completion-iteration-plan-20260920.md`

## 规划结论

- 推荐 G21–G26 六个独立 Gate，总投入 47–70 人日，4–6 人团队约 7–10 周。
- G21 必须最先恢复现有功能可信性和远端 CI；之后 G22/G24 可并行，G23 跟随 G22，G25 跟随 G24，G26 最后做整体验收。
- 不引入 HAPI、DB-GPT、LLM、第二数据面或新微服务；智能问数只执行引用已发布 Data API 的已验证问题，未知问题拒答。
- H4/H5、完整生产编排、HA、系统性压测和全量 CI 扩容继续按生产化批次纪律等待用户明示。
- 当前状态为“计划待批准”；批准后只启动 G21，不默认授权后续 Gate 连续实施。

## 验证

- [x] 计划中的现有模块、文档、迁移版本和 Data API 数据集均在仓库中核对
- [x] 六个 Gate 都可独立合并，且包含正常、失败、边界、权限、并发和回滚验收
- [x] `git diff --check` 通过
