...
- **归并格子（Unite Grid）**：把两个源身份归入同一黄金人的四分支判定（无链接建人/一方收编/已同人幂等补投影/双方并人），单一属主为 `MpiPersonService`；链接与 Doris 投影回写只经格子发生。**并人赢家统一为 earlierOf（保留创建较早者，2026-09-02 决策）**——与入口（RULE/MANUAL）和发起方向无关的确定不变量，重放/审计不依赖参数顺序；操作者要显式指定存活者走 `MpiPersonController` 的 keep/drop 合并端点。两侧仍经 `UnitePlan` 显式声明的差异：建人属性来源与决策元数据。
...
- **认证审批（Certification）**：CERTIFIED 只能经人工审批流转（V11 `ai_certification_request`：PENDING→APPROVED/REJECTED；提交须 ASSESSED 态 + readiness gate=CANDIDATE）；SERVING 发布须存在已批准认证记录。
- **评测反馈（Evaluation Feedback / 数据飞轮）**：失败样本的 Learning Plane 记录（V12 `ai_evaluation_feedback`，锚定评测明细，处置 CREATED→CONSUMED/DISMISSED）；处置只改状态——新版本（recipe/语料调整）与重评均由人工触发，候选不自动上线。
...
**失败证据形状（2026-09-02 裁决：维持聚合）**：失败规则证据消费 dbt `--store-failures` 失败表并按表形状投影——`unique`/`accepted_values` 聚合为（违规值, 次数），`not_null` 为行样本、`relationships` 为孤儿值单列。证据 JSON 不放行级业务数据；需要行级下钻时走查询面另做「取样本」能力，不回退整行样本。
