...
**失败证据形状（2026-09-02 裁决：维持聚合）**：失败规则证据消费 dbt `--store-failures` 失败表并按表形状投影——`unique`/`accepted_values` 聚合为（违规值, 次数），`not_null`/`relationships` 为行样本。证据 JSON 不放行级业务数据；需要行级下钻时走查询面另做「取样本」能力，不回退整行样本。
