...
if iptables -C DOCKER-USER -d "$DM_HOST" -p tcp --dport "$DM_PORT" -j DROP 2>/dev/null; then
    ...
    iptables -I DOCKER-USER 2 -d "$DM_HOST" -p tcp --dport "$DM_PORT" -j DROP
fi

echo "DOCKER-USER isolation applied: $EDGE_NET -> $DM_HOST:$DM_PORT allowed, other containers dropped"
