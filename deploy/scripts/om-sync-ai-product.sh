...
# 3) term（幂等 upsert：存在则 PATCH description 与 tags）
term_name = product
summary_lines = [
    f"AI Data Product：{product}",
    f"版本 {version} · Profile {profile}",
...
# 4) 认证状态标签刷新（tags 列表全量替换）
status, body = call("PUT", f"/glossaryTerms/{term_id}/tags",
                    [{"tagFQN": f"AIReadiness.{certification}"}])
print("tag refresh:", status)
