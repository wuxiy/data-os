...
/**
 * MPI 服务安全配置。骨架复制自 control-plane（OIDC resource server +
 * DISABLED 开发档），角色规则按 MPI 域调整：复核工作台读侧对治理/分析/
 * 只读角色开放，写侧（rebuild/决策/合并/拆分）仅治理线角色。
 */
@Configuration
...
public class OidcSecurityConfiguration {
    ...
    JwtDecoder jwtDecoder(AuthProperties properties) {
        if (properties.getIssuerUri() == null || properties.getIssuerUri().isBlank()) {
            throw new IllegalStateException("生产认证必须配置 DATAOS_MPI_OIDC_ISSUER_URI");
        }
        if (properties.getAudience() == null || properties.getAudience().isBlank()) {
            throw new IllegalStateException("生产认证必须配置 DATAOS_MPI_OIDC_AUDIENCE");
        }
    ...
    /**
     * Keycloak 风格的 resource roles 只从配置的 API client 采纳：其他 client
     * 的角色不得仅因出现在同一 token 中就成为 MPI 服务的权限。
     */
    ...
    /** 除 issuer 与时间性 claims 外，额外校验 OIDC audience。 */
