...
class MpiLoadBlockingTests {
    ...
    private void seedPrescriptions() {
        doris.batchUpdate("""
                ...
                // 另一机构同名同性别：B6 跨机构召回。
                new Object[] {"H0002", 5L, "张三", "男", "zz1", "9", null},
                // 缺主键 / 缺姓名：跳过计数。
    ...
    void loadNormalizesAggregatesAndSkipsInvalidIdentities() {
        ...
        assertThat(result.identitiesLoaded()).isEqualTo(4);
    ...
    void blockingFindsCardReuseAndSameNamePairsWithDedup() {
        ...
        // 手工注入跨源身份：同机构同患者主键（B3 就绪验证）。
        doris.update("""
                INSERT INTO dataos_mpi.mpi_source_identity
                  (tenant_id, institution_code, source_system, source_key, patient_id, name_norm, gender, loaded_at, updated_at)
        ...
        // B3: (EP|1,HIS|1) 跨源同主键；B6: 张三男三身份两两 3 对，其中
        // (EP|1,HIS|1) 与 B3 重叠被去重归 B3 → B6 实得 2 对。合计 6 对。
    ...
    void rebuildEndpointReturnsAggregatedStats() {
        ...
        assertThat(body.get("identitiesLoaded")).isEqualTo(4);
