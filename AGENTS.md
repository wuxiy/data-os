...
当前阶段：**生产化收口与功能迭代并行**（2026-09-03 经用户裁决自「线上内测与功能快速迭代」切换；依据与批次规划见 [docs/production-hardening-batch-plan-20260903.md](docs/production-hardening-batch-plan-20260903.md)）。核心职责：
...
4. 完善 ToB 端 API 接口；
5. 把以上能力部署到线上内测环境；
6. **按批次规划执行生产化收口**（H1–H5）。

**纪律（同等约束级）：**

- 生产化批次（H1–H5）的**启动与顺序由用户逐批次明示**；批次内工作以 gate 验收文档收口；
- 批次间歇期继续功能迭代（G 系列），两条线不混在同一提交；
- 未入批次的事项（新发现的深度测试、安全扫描、生产加固）仍记入
  [下一阶段备忘](docs/deferred-hardening-backlog.md)，不自行排期（用户明示除外）。
  **每次完成对话的工作后更新该备忘**（新增条目 / 状态变化）；
- 批次执行中发现范围需扩大时，先回备忘账与规划文档补记，再扩。
