...
- 生产构建默认是真实模式。没有真实数据服务的页面不再渲染静态样例，而是显示“待接入真实服务”并引导进入已落地的数据接入、治理驾驶舱、质量闭环、主索引、数据资产、分析看板、AI Data 和数据服务。
...
| 主索引审核 | mpi-service（门户 `/api/v1/mpi/` 直路由） | 可用，候选、复核与合并/拆分走真实服务 | 显示演示数据并标记 |
| 数据资产/技术视图 | 控制面资产/血缘 BFF（`/api/v1/assets/**`、`/api/v1/lineage/**`） | 可用；未配置 OpenMetadata 时显示待接入，不渲染样例 | 显示演示数据并标记 |
| 分析看板 | 控制面分析 BFF + Superset 嵌入（`18084` 专用监听） | 可用；未配置 Superset 时显示待接入，不渲染样例 | 显示演示数据并标记 |
| AI Data | 控制面 `/api/v1/ai-data-products` | 可用，不收录静态样例 | 显示演示边界说明（无 mock 数据） |
| 数据服务 | 控制面 `/api/v1/data-services` | 可用，不收录静态样例 | 显示演示边界说明（无 mock 数据） |
| 智能问数 | 当前为 `src/data/integrations.ts` 原型数据 | 显示待接入真实服务，不展示样例 | 显示演示回答并标记 |

资产、分析和 MPI 已分别经控制面 BFF 与 mpi-service 接入真实服务；标准、映射和问数仍为原型数据，下一步分别接入治理注册库、术语服务和问数编排 BFF。当前页面保留交互模型，但不把原型数据当作已完成能力。
...
3. DB-GPT 问数编排的服务账号及 BFF 接入（OpenMetadata、Superset 与 MPI 已接入）。
4. 责任人 Webhook 或院内消息通道。

这些依赖未就绪前，建议将当前版本定位为“数据接入、治理质量闭环、主索引、数据资产/血缘、分析看板、AI Data 与数据服务具备真实链路，标准、映射和问数为受控原型”，不要以全量医疗数据平台生产版对外承诺。
