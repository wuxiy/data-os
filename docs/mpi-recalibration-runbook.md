...
## 七、重锚基线（2026-09-03，T5b 余项，docs/validation/t5b-reanchor-20260903.md）

双流语料重锚（NEG_RATIO 0.74/0.18/0.08 + 跨流真实正样本）后的首读：**DRIFT 23/27**，
且从结构性漂移升级为真实信号——name.mAgree 1.0→0.804、gender.mDisagree 0→0.152、
tVeto 零误否界 0.42→-1.09。锚点实证：跨流真同人锚点被现行 tVeto 误否（anchor-1569541917）。
**§四 已于同日执行完毕（用户批准）**：packaged 更新（tAuto 17.62 / tReview -0.01 /
tVeto -1.09）、57/57 全绿锁新基线、dev rebuild 否决 115→26（89 个误否对回到复核）、
AUTO 360 不动。估计器新增安全审计双参形态与打包口径对齐（4 位取整后再定阈值）——
后续任何重标定都走双参形态。语料生成器负样本配额锚定合成正样本基数（跨流正样本
不放大负样本需求——超池供给会炸回填断言）。