...
> 日期：2026-09-01
> 状态：入口已裁决（2026-09-02）——G15A 由 G15 超额完成；G15B 短期 No-Go；G16/G17 语义线 parked，见 §十二
...
1. **G15A 已被实际 G15 超额完成**：不经独立影子 Gate，直接完成「影子上线 → dev 核验（26 否决全同构、零疑似误否）→ 决策权切换到 v1+v2 混合引擎」，见 `docs/validation/gate-mpi-g15-20260902.md` §五之二。
2. **G15B 记短期 No-Go**：产品准入评估确认 10 个工作日内无法获得可持续参与验收的真实业务 Owner——本计划 §三自认的最脆弱假设成立失败。按 §五 No-Go 处理：不启动语义层开发；固定看板需求交由现有 Superset 完成。
3. **G16/G17 语义线 parked**，复活条件不变：一个具名、可持续参与的业务 Owner 落实后重开 G15B 准入（指标/黄金问题清单要求仍以本计划 §五为准）。
4. **编号让渡**：G16a/G16b 编号由新主线「EP 域数据面扩展」使用（`docs/ep-domain-expansion-plan-20260902.md`）；语义线从未启动，复活时另取号，本计划内的 G16/G17 章节保留为 parked 范围定义。
5. **ProductScopeNotice 修正不随 No-Go 作废**：产品基线与已验收事实对齐（G3/G4/G6/G13）是独立于准入结论的诚实项，随新主线首个提交落地。
