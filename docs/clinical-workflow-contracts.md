...
```json
{
  ...
  "source": [{
    ...
    "query": "SELECT * FROM <source_table> WHERE update_time >= '${last_success_time}'",
...
开发机当前没有真实 LIS、EMR、手术系统主机、账号或脱敏样本，因此本版本只完成合同、凭据解析和阻断校验，不能把模板列表或 SeaTunnel 健康检查报告为业务数据已接入。
