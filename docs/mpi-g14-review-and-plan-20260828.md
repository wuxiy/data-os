...
| 决策权切换（混合策略形态） | 影子先行；纯 scored 开关未实现（评测结论：纯评分决策权不宜切换，开关将是伪选项，见 gate 报告偏差 2） | 下个 gate 依分歧清单裁决 |
...
**9/9 全过，G14 交付，备忘 T2 关闭。** 证据与偏差详见
`docs/validation/gate-mpi-g14-20260828.md`。核心结论：EP 字段集上身份信号
在字段合取而非边际——V1 合取规则零误并自动化率 0.44，纯加性 FS 仅 0.02；
V2 以影子模式上线（45/45 真实对带分数），价值在非同人复核负担 -78% 与
锚点正确的 NO_MATCH 处置。决策权保持 V1；混合策略切换列为 T5。
