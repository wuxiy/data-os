...
- 2026-09-23（G26 受控智能问数交付）：V20 两表 + assistant 模块（确定性匹配/统一拒答信封/逐次审计）+ data-api 首个 internal 路由（资源侧 JWT fail-closed、复用执行器与机构范围/熔断/审计）+ 门户问数页接真；dev 三问题与 Doris 人工 SQL 零误差、五类拒答全实测、反馈追溯落库（docs/validation/gate-g26-20260923.md，含 S-1~S-9 阶段总验收核对——G21–G26 系列收官）。dev 实抓两处老坑重现并修复：JDK HttpClient h2c 丢请求体（须 AdapterHttp 强制 HTTP/1.1）、@Value 不认 DATAOS_* 连写 env（application.yml 显式声明）。新增延后候选：① 已验证问题无管理接口（种子/改库口，DEPRECATED 下线需 DB 运维）；② 限流为单实例进程内滑窗（多副本需共享存储）；③ SERVICE_OFFLINE 拒答粒度（未知 code 与未发布不可区分）；④ 问数审计行无保留策略。
- 2026-09-23（G27 问数治理面交付）：V21 事件表 + 治理八接口（草稿生命周期 + 试运行发布门 + 停用/重新起草 + 事件留痕）+ SERVICE_OFFLINE 拒答粒度修正 + 门户专业工作区治理区块；dev 治理演练 27 项全过、粒度修正双向实证（真实 DEPRECATED 服务 vs 未知服务）、真实浏览器视觉验收通过（docs/validation/gate-g27-20260923.md）。dev 实抓部署坑一条：control-plane Dockerfile 拷预构建 jar，只 mvn test 不产包 → 镜像静默旧版（Flyway 无错不迁移）；守卫=部署前核对镜像内 jar 迁移清单。新增延后候选：① 治理列表/事件无分页；② 参数 Schema 编辑器缺 values/defaultValue 编辑入口（已有值透传保留）；③ 问数审计无管理/导出面。
- 2026-09-23（G27 余项收口）：三个延后项当轮关闭——治理/事件/审计分页（usePaged+Pager 与服务端分页）、Schema 编辑器 defaultValue/枚举入口、审计管理面（列表/过滤/CSV 导出/反馈列，角色=治理列表集）。dev 实抓并修复一处真缺陷：删除问题的事件 DB 留痕但 events 端点 requireQuestion 404，「按 code 可追溯」API 层不可达——修正为问题或事件任一存在即返回。视觉两连修（540px Drawer 换紧凑列表；.sourceList strong 特异性压 flex 的 CSS Modules 教训）。无新增延后项；审计保留策略仍留 G26 条目。
