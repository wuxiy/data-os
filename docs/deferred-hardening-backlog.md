...
| T5 | MPI 决策权混合策略与多源重标定 — **T5a 完成 2026-09-02（G15）**：混合引擎（守卫定 AUTO + 分数否决带 tVeto=0.42）三引擎冻结评测严格占优（零误并/零误否、非同人复核 -79.4%）、影子证据上线（docs/validation/gate-mpi-g15-20260902.md）；**T5b 余项**：决策权切换裁决（域决策）+ 多源接入后真实脏数据重标定 | G14 结论：加性 FS 自动化率结构性低于合取规则，切换需混合策略；评测为半合成口径，真实脏数据分布待多源验证 | T5b 生产化批 |
