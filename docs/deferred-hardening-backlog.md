...
| T5 | MPI 决策权混合策略与多源重标定 — **T5a 完成 2026-09-02（G15）含决策权切换**：混合引擎冻结评测严格占优（零误并/零误否、非同人复核 -79.4%），dev 影子核验（26 否决全同构零疑似误否）后决策权已切 `v1+v2`（docs/validation/gate-mpi-g15-20260902.md §五之二）；**T5b 余项**：多源接入后真实脏数据重标定 + 阈值漂移检测 | G14 结论：加性 FS 自动化率结构性低于合取规则，切换需混合策略；评测为半合成口径，真实脏数据分布待多源验证 | T5b 生产化批 |
